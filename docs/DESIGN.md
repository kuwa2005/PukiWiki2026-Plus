...
| **forge = 軽量 CSS のみ** | `skin/forge/` は classic コピー + CSS 調整。**React / Vite / 外部 FW 禁止** |
| **新スキン = 見た目のみ** | 配色・タイポグラフィ・余白の調整に留める。HTML 構造の大改造はしない |
...
| 項目 | 方針 |
|------|------|
| **構成** | classic コピー + `pukiwiki.css` 1 ファイル + 既存 JS（`main.js` / `search2.js`） |
| **禁止** | React / Vue / ビルドツール / CDN フレームワーク |
| **サイズ目安** | classic 同等〜数十 KB（現状 forge 全体で約 80 KB） |
| **classic 非改変** | forge 変更は `skin/forge/` + docs のみ |
