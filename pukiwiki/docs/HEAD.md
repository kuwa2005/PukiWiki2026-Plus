...
ブログ風ヘッダーには **`small` / `medium` / `large`** か、**`180`〜`320` 程度の数値**を推奨します。`6432x3900` のような原寸ピクセル数は上限2000pxに丸められるため、意図した見た目にならない場合は `#head(file,240)` のように表示高さを直接指定してください。

### 原寸画像（例: 6432×3900）について

`#head` は添付の**原寸ファイル**を `?plugin=attach` 経由で配信します（サーバー側のリサイズは行いません）。表示サイズは CSS で制御します。

- **auto**（第2引数省略）: `width:100%` + `height:auto` でペイン幅に合わせて縮小。横長（約 1.65:1）でも縦横比は維持されます。`width` / `height` HTML 属性でレイアウトシフトも抑制します。
- **cover**（数値・プリセット）: 指定高さの枠で `object-fit:cover` トリミング。ブラウザは原寸を読み込みますが、見た目の高さは 120〜320px などに固定されます。
