...
**総合所見:** 構造的 CSRF・認証・セッションの弱点は v1.0.0 で大幅に改善済み。**v1.0.1** で SEC-U01（BiDi ページ名なりすまし）を **Fixed**（#81）。残る主要リスクは本番設定依存の **HTTP セキュリティヘッダー（SEC-M06）** と NFKC 未適用（SEC-U02）。Arabic RTL override（U+202E）等の具体例は [Unicode 攻撃テストケース](#unicode--国際化攻撃テストケース) を参照。
